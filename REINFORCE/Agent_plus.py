...
for episode in tqdm(range(MAX_EPISODES)):
    ...
    # print(disc_returns)
    pred_batch = model(state_batch) # 获得模型对本次τ所有状态的预测值，即所有的P(a|S)
    prob_batch = pred_batch.gather(dim=1, index=action_batch.long().view(-1, 1)).squeeze() # 提取出概率
    # print(prob_batch)
    loss = loss_fn(prob_batch, disc_returns) # 计算-log(P(a|S))*R，直观来看，即为增强可以获得高分动作的概率（当然高分指的是最终得分乘以贴现因子。)
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
