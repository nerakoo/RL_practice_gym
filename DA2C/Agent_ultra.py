# 该改进添加了N-step机制
# 在这个机制下，critic的目标值更加准确，所以critic的训练会更稳定。
#
...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        self.actor_lin1 = nn.Linear(64,2)
...
# -------------------------run_episode-------------------------------------------
# 运行轮次并收集数据
def run_episode(worker_env, worker_model, N_steps=15):
    state_data = worker_env.env.state
    state_data = np.array(state_data)
    state = torch.from_numpy(state_data).float() # 将环境状态从Numpy数组转换为Pytorch张量
    values, logprobs, rewards = [],[],[] # 分别存储计算的状态值（critic），对数概率（actor）和奖励
    done = False
    j = 0
    G = torch.Tensor([0]) # G表示收益，初始化为0
...
# -------------------------update_params-------------------------------------------
# 更新参数
def update_params(worker_opt, values, logprobs, rewards, G, clc=0.1, gamma=0.75):
    # 处理数据，确定是一维的结构
...
def worker(t, worker_model, counter, params):
    ...
    for i in range(params['epochs']):
        ...
        values, logprobs, rewards, G = run_episode(worker_env, worker_model) # 运行轮次并收集数据
...
if __name__ == "__main__":
    ...
    env.close()
    torch.save(MasterNode, 'DA2C_N_step2.model')
