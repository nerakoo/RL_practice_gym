...
for i in tqdm(range(epochs)):
    ...
    while (status == 1):
        ...
        # 人工影响尝试，如果上一步和本步一致，则多扣一分
        if torch.equal(state_now,state_next):
            reward = reward - 1

        done = True if (reward > 0 or reward < -2) else False # 记录本步结束后是否游戏结束，如果本步结束后游戏结束，那么就不存在下一状态。(落入坑中或者到达终点)
        ...
        if (reward > 0 or reward < -2) or mov_times > max_moves: # 如果游戏结束，退出循环
            status = 0
            mov_times = 0
