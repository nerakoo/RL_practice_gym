...
def update_params(memory, model, optimizer, next_state, next_s_r, done, gamma=args.gamma):
...
def run_episode(env, model, optimizer, N_steps=35):
    state = env.reset()
    ...
    while not done:
        ...
        if epi_time % N_steps == 0 or done:
            loss = update_params(memory, model, optimizer, next_state, next_s_r, done)
            memory = []
...
def worker(t, global_model, counter, params, optimizer):
    ...
    for i in range(params.epochs):
        epi_time, epi_reward, loss = run_episode(env, global_model, optimizer)
        counter.value += 1
